...
function showPlaylistDropdown(triggerIcon, songIndex) {
    ...
    // 💡 Always show above the icon
    let top = rect.top + window.scrollY - dropdownHeight - padding;

    // Apply styles
    dropdown.style.left = `${left}px`;
    dropdown.style.top = `${Math.max(top, padding)}px`; // prevent clipping on top
    dropdown.style.maxHeight = `${dropdownHeight}px`;
    dropdown.style.overflowY = 'auto';
    dropdown.classList.remove('d-none');

    // Highlight current playlist and render list
